...
### Demo
http://auctionapp.expertweblancers.com/

## Install locally
You have to have installed node.js and npm or yarn
    
```
   git clone https://github.com/topdeveloper424/auction-app.git
   cd auction-app
   yarn install
   yarn start
```
...
* `src/index.spec.js` is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/index.spec.js`, but you could always remove that import if you later migrate to a different build tool.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the ```src/index.spec.js``` file:
